...
var jobErrorTestData = struct {
	...
	// Job functions
	fn: func(tx *gorm.DB, deployment *Deployment, value interface{}) (interface{}, error) {
		return value, nil
	},
	failingFn: func(tx *gorm.DB, deployment *Deployment, value interface{}) (interface{}, error) {
		return value, errors.New("fn")
	},

	// Job error handlers
	errHandler: func(tx *gorm.DB, deployment *Deployment, value interface{}, err error) (interface{}, error) {
		return value, nil
	},
	passthroughErrHandler: func(tx *gorm.DB, deployment *Deployment, value interface{}, err error) (interface{}, error) {
		return value, err
	},
	failingErrHandler: func(tx *gorm.DB, deployment *Deployment, value interface{}, err error) (interface{}, error) {
...
func TestErrorHandlerIgnoreError(t *testing.T) {
	...
	test := func(fn JobFunc) {
		...
		_, err := wrappedFn(tr.db, deployment, nil)
...
func TestJobRunErrorHandler(t *testing.T) {
	...
	test := func(job *Job, expectedErr error) {
		_, err := job.Run(tr.db, deployment, nil)
