...
import (
	"context"
	"fmt"
	"gitlab.com/ignitionrobotics/web/cloudsim/pkg/application/requester"
...
func (app *application) Requester() requester.Requesters {
	return &app.requesters
}
...
func New(p platform.Platform, simulationService simulations.Service, userService users.Service) Application {
	app := &application{
		...
		requesters: requesters{
			start:    requester.NewStartRequester(),
			launch:   requester.NewLaunchRequester(),
			restart:  requester.NewRestartRequester(),
			shutdown: requester.NewShutdownRequester(),
		},
	}
	return app
}
