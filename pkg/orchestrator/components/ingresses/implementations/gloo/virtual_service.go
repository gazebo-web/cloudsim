...
// VirtualServices is an ingresses.Ingresses implementation using Gloo.
...
func (v *VirtualServices) Get(name string, namespace string) (resource.Resource, error) {
	...
	s := resource.NewSelector(vs.Labels)
	return resource.NewResource(vs.Name, vs.Namespace, s), nil
}

// GetUpstream is used to get the matching upstream for a certain service in the cluster identified by the given selector.
func (v *VirtualServices) GetUpstream(namespace string, selector resource.Selector) (resource.Resource, error) {
	v.Logger.Debug(
	...
	s := resource.NewSelector(list.Items[0].Labels)
	res := resource.NewResource(list.Items[0].Name, namespace, s)
	return res, nil
}

// NewVirtualServices initializes a new ingresses.Ingresses implementation using Gloo Virtual Services.
func NewVirtualServices(gw gateway.GatewayV1Interface, logger ign.Logger, client gloo.GlooV1Interface) ingresses.Ingresses {
