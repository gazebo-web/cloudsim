...
func TestNewVirtualHosts(t *testing.T) {
	var vhs ingresses.IngressRules
...
func setupTestVirtualHosts(t *testing.T, regex string) (*gatewayFake.FakeGatewayV1, ign.Logger, resource.Resource) {
	// Define constants
...
func TestVirtualHost_Remove(t *testing.T) {
	...
	err = vhs.Remove(rule, p...)
	assert.Error(t, err)
	assert.Equal(t, err, ingresses.ErrRuleEmpty)
}
