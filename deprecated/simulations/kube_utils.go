...
func KubernetesWeaveRemovePeer(ctx context.Context, kc kubernetes.Interface, node *apiv1.Node) error {
	...
	if options, err := KubernetesPodExec(ctx, kc, metav1.NamespaceSystem, weavePodName, "weave", command, nil); err != nil {
		msg := createKubernetesPodExecErrorMsg(
